...
df_data_stock = df_data_stock.withColumn("Year", when(col("Year") == "", year(current_date())).otherwise(col("Year")))
...
df_data_stock = df_data_stock.withColumn("Actual_Date", when(col("Days_Ago").isNotNull(), expr("date_sub(current_date(), Days_Ago)"))
...
df_data_stock = df_data_stock.drop("Date", "Days_Ago", "Day", "Month", "Year", "Month_Num", "Full_Date", "Actual_Date")
